define([],function(){
  ...
  city.init=true;
  city.proSel=city.citySel=false;//是否默认选择省
  city.initSel={};
  city.initSel.county=city.initSel.city="";
  city.change=function($areaSelect,arrIdx) {
      var idx,arrIdx=arrIdx||0,_=this,html="",queryArr=[],str="";
      if(city.init){
        if(!$areaSelect.data("queryArr")) $areaSelect.data("queryArr",[]),$areaSelect.data("str","0");
        $areaSelect.data("queryArr")&&$areaSelect.data("queryArr").push($areaSelect.data("str"));
      }
      queryArr=$areaSelect.data("queryArr");
      str=$areaSelect.data("str");
      $.each(dsy.Items[queryArr[arrIdx]],function(k,v){
        if(new RegExp($.trim($areaSelect.find(".selectBox:eq("+arrIdx+") .head span").text())).test(v)){
          idx=k;
        };
      });
      $.each(dsy.Items[queryArr[arrIdx]],function(k,v){
        if(arrIdx==1&&k==0) city.initSel.city=v;
        if(arrIdx==2&&k==0) city.initSel.county=v;
        if(arrIdx==0&&k>3&&k<31){
          html+="<li class='reset'><b>"+v+"</b></li>";
        }else{
          html+="<li><b>"+v+"</b></li>"
        }
      });
      // $(".areaSelect .selectBox:eq("+arrIdx+") .body ul").html("<li><b>默认</b></li>"+html);
      $areaSelect.find(".selectBox:eq("+arrIdx+") .body ul").html(html);
      if(!city.init){//not init contions 
        // city.proSel&&$(".areaSelect .selectBox:eq(2) ul").html("<li><b>默认</b></li>");
        city.proSel&&$areaSelect.find(".selectBox:eq(1) .head span").text(city.initSel.city);
        $areaSelect.find(".selectBox:eq(2) .head span").text(city.initSel.county);
      };
      str+="_"+idx;
      $areaSelect.data("str",str)
      arrIdx++;
      if(arrIdx<3) city.change($areaSelect,arrIdx);
  };
  var dsy = new Dsy();
