    ...
    b.DEFAULTS = { interval: 5e3, pause: "hover", wrap: !0 }, b.prototype.cycle = function(b) {
        ...
        if (!e.hasClass("active")) {
            if (this.$indicators.length && (this.$indicators.find(".active").removeClass("active"), this.$element.one("slid.bs.carousel", function() {
                ...
                e.addClass(b), e[0].offsetWidth, d.addClass(g), e.addClass(g), d.one(a.support.transition.end, function() { e.removeClass([b, g].join(" ")).addClass("active"), d.removeClass(["active", g].join(" ")), i.sliding = !1;var $activeItem=$("#myCarousel .item.active"),$close=$("#myCarousel .closeBtn");
		          for(var num=0;num<10;num++){
		            $close.removeClass("close_"+num);
		            if($activeItem.find(".model_"+num).length) $close.addClass("close_"+num);
		          };
          setTimeout(function() { i.$element.trigger("slid.bs.carousel") }, 0) }).emulateTransitionEnd(600) } else {
                if (this.$element.trigger(j), j.isDefaultPrevented()) return;
